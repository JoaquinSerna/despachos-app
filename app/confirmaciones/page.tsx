...
// Normalización para comparar clientes y direcciones
...
function detectarPropuestas(pedidos: Pedido[], rechazadas: Set<string>): PropuestaGrupo[] {
  ...
  function find(id: string): string {
    if (parent[id] !== id) parent[id] = find(parent[id]); return parent[id]
  }
  ...
  for (let i = 0; i < sinGrupo.length; i++) {
    for (let j = i + 1; j < sinGrupo.length; j++) {
      ...
      if (tokenesSimilares(a.cliente, b.cliente) && tokenesSimilares(a.direccion, b.direccion))
        union(a.id, b.id)
    }
  }
...
// ─── Constructor de mensajes WA ───────────────────────────────────────────────

function buildWAMessages(
  tipo: WATipo, pedidos: Pedido[],
  itemsMap: Record<string, PedidoItem[]>,
  franja: string, fechaReprogLabel: string
): string {
  ...
  if (!multi) {
    ...
    switch (tipo) {
      ...
      case 'no_contesto':
        return `Hola ${nombre}! 👋 Nos comunicamos de Construyo al Costo para confirmar la entrega de tu pedido NV ${p.nv}, que incluye:\n${its}\nPor favor respondé este mensaje para confirmar. En caso de no recibir respuesta, el pedido quedará reprogramado para el día siguiente.\n¡Gracias!`
    }
  }

  // Mensaje compuesto (grupo)
  ...
  const footer = {
    aviso: '\n¿Confirmás que vas a poder recibirlos?',
    confirmado: '\nAnte cualquier consulta estamos a disposición.',
    ya_salio: '\nPor favor asegurate de tener alguien disponible para recibirlos.',
    reprog_cliente: '\nLuego nos volveremos a comunicar para reconfirmar los horarios.',
    reprog_nuestro: '\nLuego nos volveremos a comunicar para reconfirmar los horarios.',
    no_contesto: '\nPor favor respondá este mensaje para confirmar. En caso de no recibir respuesta, los pedidos quedarán reprogramados para el día siguiente.\n¡Gracias!',
  }[tipo]
...
function BadgeConfirmacion({ pedido }: { pedido: Pedido }) {
  if (pedido.estado === 'en_camino')
    return <span className="text-xs px-2 py-1 rounded-full font-medium" style={{ background: '#dbeafe', color: '#1d4ed8' }}>🚛 En camino</span>
  if (pedido.confirmado_cliente)
    return <span className="text-xs px-2 py-1 rounded-full font-medium" style={{ background: '#d1fae5', color: '#065f46' }}>✓ Confirmado</span>
  if (pedido.confirmacion_estado === 'rechazado_cliente')
    return <span className="text-xs px-2 py-1 rounded-full font-medium" style={{ background: '#fde8e8', color: '#E52322' }}>🚫 Rechazado (cliente)</span>
  if (pedido.confirmacion_estado === 'rechazado_cac')
    return <span className="text-xs px-2 py-1 rounded-full font-medium" style={{ background: '#fde8e8', color: '#E52322' }}>🚫 Rechazado (CAC)</span>
  if (pedido.confirmacion_estado === 'no_contesto')
    return <span className="text-xs px-2 py-1 rounded-full font-medium" style={{ background: '#fef3c7', color: '#b45309' }}>📵 No contestó</span>
...
export default function ConfirmacionesPage() {
  ...
  const [agrupando, setAgrupando] = useState<string | null>(null) // key de grupo en proceso
  ...
  // Detectar propuestas de grupo cuando cambian los pedidos
  useEffect(() => {
    if (pedidos.length > 1) {
      const found = detectarPropuestas(pedidos, propuestasDesc)
      setPropuestas(found)
    }
  }, [pedidos, propuestasDesc])

  const cargarPedidos = async (params?: {
    fecha?: string; sucursal?: string; confirmado?: 'todos' | 'confirmado' | 'sin_confirmar'
  }) => {
    setCargando(true)
    ...
    // Query 1: programado + en_camino del día
    let q1 = supabase
      .from('pedidos').select(campos)
    ...
    // Query 2: rechazados del día (ya reprogramados)
    let rechazados: Pedido[] = []
    if (fecha && confirmado !== 'confirmado') {
      let q2 = supabase.from('pedidos').select(campos)
  ...
  const marcarNoContesto = async (pedidoId: string) => {
    ...
    const pedido = pedidos.find(p => p.id === pedidoId)
    const { error } = await supabase.from('pedidos')
      .update({ confirmacion_estado: 'no_contesto', fecha_confirmacion: hoy() }).eq('id', pedidoId)
    if (error) { showToast('Error', 'err') }
    else {
  ...
  const marcarEnCamino = async (pedido: Pedido) => {
    ...
    if (error) { showToast('Error al marcar en camino', 'err'); setMarcandoEnCamino(null); return }
    setPedidos(prev => prev.map(p => p.id === pedido.id ? { ...p, estado: 'en_camino' } : p))
    showToast('Marcado en camino 🚛')
    if (usuario) logAuditoria(usuario.id, nombreUsuario, 'Marcó en camino (manual)', 'Confirmaciones', { nv: pedido.nv, cliente: pedido.cliente })
    setMarcandoEnCamino(null)
    // Abrir WA con "ya salió" preseleccionado
  ...
  const guardarDireccion = async (pedidoId: string, valor: string) => {
    ...
    if (error) { showToast('Error al guardar dirección', 'err') }
      ...
      if (usuario && pedido) logAuditoria(usuario.id, nombreUsuario, 'Actualizó dirección', 'Confirmaciones', { nv: pedido.nv, direccion_nueva: valor.trim() })
    }
  }
  ...
  const confirmarReprog = async () => {
    ...
    const nota = `⚡ Reprog. desde ${pedido.fecha_entrega} V${pedido.vuelta} — ${reprogMotivo === 'cliente' ? 'a pedido del cliente' : 'reprogramado por CAC'}`
    ...
    if (!res.ok) { showToast(`Error: ${data.error ?? 'desconocido'}`, 'err'); return }
    ...
    if (usuario) logAuditoria(usuario.id, nombreUsuario, 'Rechazó y reprogramó pedido', 'Confirmaciones', { nv: pedido.nv, fecha_nueva: savedFecha, motivo: savedMotivo })
    setModalReprog(null)
    showToast('Pedido reprogramado — recordá enviar el WA')
    await abrirModalWA([{ ...pedido }], savedMotivo === 'cliente' ? 'reprog_cliente' : 'reprog_nuestro', savedFecha, savedVuelta, savedMotivo)
  }

  const abrirModalWA = async (
    pedidosList: Pedido[], tipoPresel?: WATipo,
    fechaReprog?: string, vueltaReprog?: number, motivoReprog?: 'cliente' | 'nosotros'
  ) => {
    const tipo = tipoPresel ?? 'aviso'
    setWATipo(tipo)
    setWAFranja(vueltaAFranja(pedidosList[0].vuelta))

    // Items cacheados para todos los pedidos del grupo
    ...
    if (sinCache.length > 0) {
      ...
      for (const p of sinCache) {
        ...
        newMap[p.id] = items
        setItemsCache(prev => ({ ...prev, [p.id]: items }))
      }
  ...
  const aprobarGrupo = async (pedidoIds: string[], key: string) => {
    ...
    const { error } = await supabase.from('pedidos')
      .update({ grupo_confirmacion: grupoId })
      .in('id', pedidoIds)
  ...
  const desagrupar = async (grupoId: string) => {
    const { error } = await supabase.from('pedidos')
      .update({ grupo_confirmacion: null })
      .eq('grupo_confirmacion', grupoId)
    if (!error) {
      setPedidos(prev => prev.map(p => p.grupo_confirmacion === grupoId ? { ...p, grupo_confirmacion: null } : p))
      showToast('Grupo separado')
    }
  }

  // ─── Agrupación de cards para renderizar ───────────────────────────────────
  ...
  const totalPendientes = pedidos.filter(p => !p.confirmado_cliente && !p.confirmacion_estado && p.estado !== 'en_camino').length
  ...
  const waPreview = modalWA
    ? buildWAMessages(waTipo, modalWA.pedidos, modalWA.itemsMap, waFranja,
        modalWA.fechaReprog ? formatFechaCorta(modalWA.fechaReprog) : '')
    : ''

  // Teléfono para el grupo (primer pedido con teléfono)
  const telefonoGrupo = (peds: Pedido[]) => peds.find(p => p.telefono)?.telefono ?? null

  // Propuestas activas (no descartadas)
      ...
      {modalReprog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
            ...
            <p className="text-xs mb-4" style={{ color: '#B9BBB7' }}>NV {modalReprog.pedido.nv} · {modalReprog.pedido.cliente}</p>
      ...
      {modalGrupos && propuestasActivas.length > 0 && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md flex flex-col" style={{ maxHeight: '80vh' }}>
            <div className="px-5 py-4 flex items-center justify-between shrink-0" style={{ borderBottom: '1px solid #f0f0f0' }}>
              <div>
                <p className="font-semibold text-sm" style={{ color: '#1a1a1a' }}>👥 Posibles grupos detectados</p>
                <p className="text-xs mt-0.5" style={{ color: '#B9BBB7' }}>Confirma o descarta cada agrupación</p>
              </div>
              <button onClick={() => setModalGrupos(false)} className="text-xl leading-none px-2" style={{ color: '#B9BBB7' }}>×</button>
            </div>
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              {propuestasActivas.map(prop => {
                ...
                return (
                  <div key={prop.key} className="rounded-xl p-3 border" style={{ borderColor: '#e8edf8', background: '#f9faff' }}>
                    <p className="text-xs font-semibold mb-0.5" style={{ color: '#254A96' }}>{ps[0].cliente}</p>
                    <p className="text-xs mb-2" style={{ color: '#B9BBB7' }}>📍 {ps[0].direccion}</p>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => aprobarGrupo(prop.pedidoIds, prop.key)} disabled={agrupando === prop.key}
                        className="flex-1 py-2 rounded-xl text-xs font-semibold text-white disabled:opacity-40"
                        style={{ background: '#254A96' }}>
                        {agrupando === prop.key ? '...' : '👥 Agrupar'}
                      </button>
                      <button onClick={() => { setPropuestasDesc(prev => new Set([...prev, prop.key])) }}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full flex flex-col" style={{ maxWidth: 440, maxHeight: '90vh' }}>
            <div className="px-5 py-4 flex items-start justify-between shrink-0" style={{ borderBottom: '1px solid #f0f0f0' }}>
              <div>
                <p className="font-semibold text-sm" style={{ color: '#1a1a1a' }}>
                  💬 WhatsApp{modalWA.pedidos.length > 1 ? ` · ${modalWA.pedidos.length} pedidos` : ''}
                </p>
                <p className="text-xs mt-0.5" style={{ color: '#B9BBB7' }}>
                  {modalWA.pedidos[0].cliente}
                  {modalWA.pedidos.length > 1
                    ? ` · NV ${modalWA.pedidos.map(p => p.nv).join(', ')}`
                    : ` · NV ${modalWA.pedidos[0].nv}`}
                </p>
              </div>
            ...
            <div className="px-5 py-4 space-y-1.5 shrink-0" style={{ borderBottom: '1px solid #f0f0f0' }}>
              ...
              {/* Selector franja — solo para aviso y confirmado en pedidos simples */}
            ...
            <div className="px-5 py-4 flex-1 overflow-y-auto min-h-0">
              ...
              {loadingItems ? (
                <div className="flex items-center gap-2 py-4">
                  <div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: '#254A96', borderTopColor: 'transparent' }} />
                ...
                <pre className="text-xs leading-relaxed whitespace-pre-wrap rounded-xl p-3"
                  style={{ background: '#f4f4f3', color: '#1a1a1a', fontFamily: 'inherit' }}>{waPreview}</pre>
              )}
            </div>
            ...
            <div className="px-5 py-4 flex gap-2 shrink-0" style={{ borderTop: '1px solid #f0f0f0' }}>
              ...
              {(() => {
                const tel = telefonoGrupo(modalWA.pedidos)
                return tel ? (
                  <a href={`https://wa.me/${formatWANumber(tel)}?text=${encodeURIComponent(waPreview)}`}
          ...
          <div className="mb-4 rounded-xl px-4 py-3 flex items-center justify-between gap-3"
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm" style={{ color: '#0369a1' }}>
                <strong>{propuestasActivas.length} posible{propuestasActivas.length > 1 ? 's' : ''} agrupación{propuestasActivas.length > 1 ? 'es' : ''}</strong> de pedidos del mismo cliente y dirección
              </p>
            </div>
            <div className="flex gap-2 shrink-0">
              <button onClick={() => setModalGrupos(true)}
          ...
          <div className="space-y-6">
            {Object.entries(porFecha).sort(([a], [b]) => a.localeCompare(b)).map(([fecha, pedidosDia]) => {
              ...
              pedidosDia.forEach(p => {
                if (p.grupo_confirmacion) {
                  if (!gruposMap[p.grupo_confirmacion]) gruposMap[p.grupo_confirmacion] = []
                  gruposMap[p.grupo_confirmacion].push(p)
                } else { sueltos.push(p) }
              })
              ...
              return (
                <div key={fecha}>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-xs font-semibold px-3 py-1 rounded-full capitalize" style={{ background: '#e8edf8', color: '#254A96' }}>
                      {formatFecha(fecha)}
                    </span>
                    <div className="h-px flex-1" style={{ background: '#e8edf8' }} />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                    ...
                    {Object.entries(gruposMap).map(([grupoId, gPedidos]) => (
                      <GrupoCard
                        ...
                        marcandoEnCamino={marcandoEnCamino}
                        editDirecciones={editDirecciones}
                        onConfirmar={confirmarCliente}
                        onDesconfirmar={desconfirmarCliente}
                        onNoContesto={marcarNoContesto}
                        onEnCamino={marcarEnCamino}
                        onRechazar={(p) => { setReprogFecha(''); setReprogVuelta(1); setReprogMotivo('cliente'); setModalReprog({ pedido: p }) }}
                        onWA={(peds) => abrirModalWA(peds)}
                        onDesagrupar={() => desagrupar(grupoId)}
                        onDeshacerEstado={deshacerEstado}
                    ...
                    {sueltos.map(pedido => (
                      <PedidoCard
                        ...
                        onConfirmar={() => confirmarCliente(pedido.id)}
                        onDesconfirmar={() => desconfirmarCliente(pedido.id)}
                        onNoContesto={() => marcarNoContesto(pedido.id)}
                        onEnCamino={() => marcarEnCamino(pedido)}
                        onRechazar={() => { setReprogFecha(''); setReprogVuelta(1); setReprogMotivo('cliente'); setModalReprog({ pedido }) }}
                        onWA={() => abrirModalWA([pedido])}
                        onDeshacerEstado={() => deshacerEstado(pedido.id)}
...
function PedidoCard({ pedido, confirmando, marcandoEnCamino, editDireccion, onConfirmar, onDesconfirmar, onNoContesto, onEnCamino, onRechazar, onWA, onDeshacerEstado, onEditDireccion, onGuardarDireccion }: {
  ...
  const borderColor = pedido.confirmado_cliente ? '#d1fae5' : esEnCamino ? '#93c5fd' : esRechazado ? '#fca5a5' : esNoContesto ? '#fde68a' : '#f0f0f0'
  const bgHeader = pedido.confirmado_cliente ? '#f0fdf4' : esEnCamino ? '#eff6ff' : esRechazado ? '#fff5f5' : esNoContesto ? '#fffbeb' : 'white'
    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden" style={{ border: `2px solid ${borderColor}` }}>
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
              ...
              <span className="text-xs px-1.5 py-0.5 rounded-full font-medium" style={{ background: '#e8edf8', color: '#254A96' }}>
              ...
              {pedido.estado_pago === 'pago_en_obra' && (
                <span className="text-xs px-1.5 py-0.5 rounded font-semibold" style={{ background: '#ffedd5', color: '#9a3412' }}>💰 P.Obra</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="px-4 py-3 space-y-2.5" style={{ borderTop: '1px solid #f4f4f3' }}>
        ...
        <div className="flex items-start gap-2">
          ...
          <input type="text"
            value={editDireccion ?? pedido.direccion}
            onChange={e => onEditDireccion(e.target.value)}
            onBlur={e => onGuardarDireccion(e.target.value)}
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={onWA} className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold shrink-0"
        ...
        {pedido.confirmado_cliente ? (
          ...
          <button onClick={onDeshacerEstado}
          ...
          <button onClick={onDeshacerEstado}
            className="w-full py-2 rounded-xl text-xs font-medium"
            style={{ background: '#f4f4f3', color: '#B9BBB7' }}>Deshacer</button>
        ) : (
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <button onClick={onEnCamino} disabled={marcandoEnCamino === pedido.id}
                ...
                {marcandoEnCamino === pedido.id ? '...' : '🚛 En camino'}
              </button>
            </div>
          </div>
        )}
...
function GrupoCard({ grupoId, pedidos, confirmando, marcandoEnCamino, editDirecciones, onConfirmar, onDesconfirmar, onNoContesto, onEnCamino, onRechazar, onWA, onDesagrupar, onDeshacerEstado, onEditDireccion, onGuardarDireccion }: {
  grupoId: string; pedidos: Pedido[]
  confirmando: string | null; marcandoEnCamino: string | null
  editDirecciones: Record<string, string>
  ...
  const allEnCamino = pedidos.every(p => p.estado === 'en_camino')
    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden col-span-1 md:col-span-2"
      ...
      <div className="px-4 py-3 flex items-center justify-between" style={{ background: '#eef2ff' }}>
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-semibold text-sm truncate" style={{ color: '#1a1a1a' }}>{pedidos[0].cliente}</p>
            <p className="text-xs truncate" style={{ color: '#6366f1' }}>📍 {pedidos[0].direccion} · {pedidos.length} pedidos</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0 ml-2">
          {tel
            ? <a href={`tel:${tel}`} className="text-xs font-medium" style={{ color: '#4f46e5' }}>{tel}</a>
            : <span className="text-xs" style={{ color: '#B9BBB7' }}>Sin tel.</span>
          }
          <button onClick={() => onWA(pedidos)}
            className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold"
            style={{ background: '#dcfce7', color: '#166534' }}>💬 WA grupo</button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3">
        {pedidos.map(pedido => (
          <PedidoCard
            ...
            onDesconfirmar={() => onDesconfirmar(pedido.id)}
            onNoContesto={() => onNoContesto(pedido.id)}
            onEnCamino={() => onEnCamino(pedido)}
            onRechazar={() => onRechazar(pedido)}
            onWA={() => onWA([pedido])}
            onDeshacerEstado={() => onDeshacerEstado(pedido.id)}
